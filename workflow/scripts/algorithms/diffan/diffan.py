...
# Make the imports below work both when this file is executed via Snakemake
# and when it is imported from elsewhere (e.g. the DOTS package or notebooks).
...
from gaussian_diffusion import GaussianDiffusion, UniformSampler, get_named_beta_schedule, \
...
class DiffAN():
    ...
    def fit(self, X, constraint=False, inst_const=True, nb_timesteps=-1, nb_variables=-1, norm=True):
        if norm:
